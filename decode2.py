...
# harpoonClose(fo, "Decode2coarse.cas", DesiredyPlus, Chord/1000.0, rho_C,\
#V_L, Visc_C, RefLev, FirstCellHtMult)
harpoonClose(fo, "Decode2fine.cas", DesiredyPlus, Chord/1000.0, rho_C,\
V_L, Visc_C, RefLev, FirstCellHtMult)
fo.close()
if 1: # set to 1 to write out files but 0 for wing optimization
    ...
    fo = open("Decode2_XFLR5_plane.xml", "w")
    Posn=0.0
...
# now write out matching Fluent journal files
    ...
    fo = open("C:\\Users\\Andy\\Documents\\airconicsv021\\Decode2_SA_coarse_15ms.jou", "w")
    for AoA in range(2,16,2):
        fluentData(fo, "Decode2coarse.cas", Awing/1000000.0, rho_C, Visc_C, Chord, 0.0, Vmax_C)
        ...
        fluentCalc(fo, 500, 0.2)
        fluentRunOn(fo, 1500)
        fluentReport(fo, "Decode2_SA_coarse")
    fo.close()
    
    fo = open("Decode2_Ke_coarse_30ms.jou", "w")
    for AoA in range(2,16,2):
        fluentData(fo, "Decode2coarse.cas", Awing/1000000.0, rho_C, Visc_C, Chord, 0.0, Vmax_C)
        fluentInitc(fo, Vmax_C, AoA)
        if (AoA > -8):
        ...
        fluentCalc(fo, 500, 0.2)
        fluentRunOn(fo, 1500)
        fluentReport(fo, "Decode2_Ke_coarse")
    fo.close()
    
    fo = open("Decode2_Kw_coarse_30ms.jou", "w")
    for AoA in range(-12,8,2):
        ...
        fluentCalc(fo, 500, 0.2)
        fluentRunOn(fo, 1500)
        fluentReport(fo, "Decode2_Kw_coarse")
    fo.close()
    
    
    fo = open("Decode2_SA_fine_30ms.jou", "w")
    for AoA in range(2,16,2):
        fluentData(fo, "Decode2fine.cas", Awing/1000000.0, rho_C, Visc_C, Chord, 0.0, Vmax_C)
        fluentModel(fo, 0) # model 0 is SA while 1 is Ke and 2 is Kw
        fluentBCs(fo, Vmax_C, AoA, "rhinoceros_binary_stl___aug_10_2015.1", 0)
        fluentMethod(fo, 0, 0) # model and method, method 0 is first order while 1 is second order
        fluentInitc(fo, Vmax_C, AoA)
        ...
        fluentCalc(fo, 1500, 0.2)
        fluentRunOn(fo, 1500)
        fluentReport(fo, "Decode2_SA_fine")
    fo.close()
    
    fo = open("Decode2_Ke_fine_30ms.jou", "w")
    for AoA in range(2,16,2):
        fluentData(fo, "Decode2fine.cas", Awing/1000000.0, rho_C, Visc_C, Chord, 0.0, Vmax_C)
        fluentInitc(fo, Vmax_C, AoA)
        if (AoA > -8):
        ...
        fluentCalc(fo, 1500, 0.2)
        fluentRunOn(fo, 1500)
        fluentReport(fo, "Decode2_Ke_fine")
    fo.close()
    
    fo = open("Decode2_Kw_fine_30ms.jou", "w")
    for AoA in range(-4,18,1):
        ...
        fluentCalc(fo, 500, 0.2)
        fluentRunOn(fo, 1000)
        fluentReport(fo, "Decode2_Kw_fine")
    fo.close()
